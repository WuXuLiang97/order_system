...
func GetProductOutboundItems(outboundID int) ([]ProductOutboundItem, error) {
	rows, err := DB.Query(`
        SELECT id, outbound_id, product_id, product_name, spec, unit, quantity
	...
	for rows.Next() {
		...
		if err := rows.Scan(&it.ID, &it.OutboundID, &it.ProductID, &it.ProductName, &it.Spec, &it.Unit, &it.Quantity); err != nil {
			return nil, err
		}
		list = append(list, it)
	}
